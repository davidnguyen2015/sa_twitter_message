...
class Window(Frame):
    ...
    def init_window(self):
        ...
        self.chk_CNN = Radiobutton(self, text="CNN", variable=self.neural_check, font=("arial", 15), value=1)
        self.chk_CNN.grid(row=1, column=0, sticky=W)

        self.chk_RNN = Radiobutton(self, text="RNN", variable=self.neural_check, font=("arial", 15), value=2)
        self.chk_RNN.grid(row=2, column=0, sticky=W)

        self.chk_CNN_RNN_Pra = Radiobutton(self, text="CNN and RNN (parallel)", variable=self.neural_check, font=("arial", 15), value=3)
        self.chk_CNN_RNN_Pra.grid(row=3, column=0, sticky=W)

        self.chk_CNN_RNN_Seq = Radiobutton(self, text="CNN and RNN (sequence)", variable=self.neural_check, font=("arial", 15), value=4)
